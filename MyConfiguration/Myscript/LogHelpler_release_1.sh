...
#   methods of all classes of a java file. It can deal with file or directroy. When you run this script for the first time, 
#   the original file will be copied to ~/.LogHelpler/src (~/.LogHelpler/src will be created if it doesn't exist), and when you run it again 
#   to handle the same original file in the future, the backup will update to keep trace with the lastest modifies of the original 
#   file before any transaction. However, PLEASE BACKUP THE ORIGINAL FILE MANUALLY IN CASE OF DATA LOSS!!!
#
#   If you want to add additional Log.d() statement for some purpose in specific method, don't use the same format as the script used, 
#   namely Log.d("...", "*****##### Enter/Leave/Create ... #####*****), because this script support option "clean" which will delete 
#   all lines in this format. If you meet some errors when use the file, you can use option "debug" to debug
# 
#   Sometimes, errors will encounter when compile the file treated by this script, usually "unreachedable statement", go to the point, and delete
...
#   directroy: all .java file in the directroy will be treated.
...
#           sudo apt-get install ctags-exuberant (if it doesn't work please run "sudo apt-get install ctags" instead.)
#
# History:
#   2013-11-30  gary    First release
#
# Bugs:
...
tput setf 4
# update log file, if exist, delete it and recreate it.
declare LOGFILE
LOGFILE="${DESDIR}/log/${1##*/}"
LOGFILE="${LOGFILE%.*}_log"
if [ -f "${LOGFILE}" ]; then
    rm -f ${LOGFILE}
    echo "Delete $LOGFILE" > ${LOGFILE}
else
    touch $LOGFILE
fi
...
declare TMPUNREACHEDABLEFILE="/tmp/.LogHelplerunreachedfile"
...
if [ -f "${SRCFILE}" ]; then
    rm -f ${SRCFILE}
    echo "Delete ${SRCFILE}" >> ${LOGFILE}
fi
if [ -f "${TAGFILE}" ]; then
    rm -f ${TAGFILE}
    echo "Delete ${TAGFILE}" >> ${LOGFILE}
fi
if [ -f "${METHODFILE}" ]; then
    rm -f ${METHODFILE}
    echo "Delete ${METHODFILE}" >> ${LOGFILE}
fi
...
# echo "Enter unreachedMethod"
    ...
    # echo "there are $wl records in $TMPUNREACHEDABLEFILE"
    if [ $wl -gt 0 ]; then
        tput setf 2
...
if [ $# -lt 1 ]; then
    echo "No file has been specified, do nothing." >> ${LOGFILE}
    echo "No file has been specified, do nothing."
    tput setf 7
    exit 1
elif [ $# -gt 2 ]; then
    echo "Only one file can be handled at one time." >> ${LOGFILE}
    echo "Only one file can be handled at one time."
    tput setf 7
    exit 1
elif [ -f $1 ]; then
    if [ -z "$(echo $1 | grep '.*\.java$')" ]; then
        echo "$1 is not a java file." >> ${LOGFILE}
        ...
        sed -r -i '/Log\.d\(".*", "\*\*\*\*\*##### (Enter|Leave|Create).* #####\*\*\*\*\*"\);|added by LogHelpler from gary/d' $1
    ...
    rm -f $TMPUNREACHEDABLEFILE
    touch $TMPUNREACHEDABLEFILE
    FILESUM=$(find $1 -name "*.java" | wc -l | awk '{print $1}')
    ...
    echo "TotalTime for whole directroy: $((($dEndtime - $dStarttime) / 3600))h $((($dEndtime - $dStarttime) % 3600 / 60))m $((($dEndtime - $dStarttime) % 60))s" >> $LOGFILE
    tput setf 7
    unreachedMethod
    declare var
    for var in $(find $1 -type f -name "*.java")
    do
        rm -f /tmp/.hejinyiLogHelplertmp
        touch /tmp/.hejinyiLogHelplertmp
        awk '{gsub(/garymethodline|garycreateline/,NR)}{print $0}' $var >> /tmp/.hejinyiLogHelplertmp
        mv /tmp/.hejinyiLogHelplertmp $var
    done
    exit 0
fi
tput setf 7
...
echo "************************************************************" >> ${LOGFILE}
sed -r -i '/Log\.d\(".*", "\*\*\*\*\*##### (Enter|Leave|Create).* #####\*\*\*\*\*"\);|added by LogHelpler from gary/d' $1
cp -v $1 ${DESDIR}/src >> ${LOGFILE}
if [ $DEBUG == "true" ]; then
    echo "copy the lastest version of $1 to ${DESDIR}/src"
    echo "copy the lastest version of $1 to ${DESDIR}/src" >> $LOGFILE
fi
if [ $RECURSION -eq 0 ]; then
    count=$(tput cols)
    declare -i var=1
...
sed -i '/\/\*.*\*\//s/\/\*/"hejinyi\*/g' $1
sed -i '/"hejinyi\*.*\*\//s/\*\//\*hejinyi"/g' $1
sed -i '/\/\*/,/\*\//s/^/\/\/hejinyicomment/g' $1
...
tput setf 2
echo -e "Linesofmethodfile = $LINESOFMETHODFILE, Linesofsourcefile = $LINESOFSOURCEFILE\n" >> ${LOGFILE}
tput setf 7
...
function findBlock() {
    ...
    if [ "$TYPE" == "methodBlock" ]; then
        ...
        CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep '[;{]')
        while [ -z "${CONTENTVAR}" ]
        do
            LINE=$((${LINE} + 1))
            CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep '[;{]')
        ...
        if [ -z "$(echo ${CONTENTVAR} | grep '{')" ]; then
            ...
            LINEBACKUP=$LINE
            LINE=$((${LINE} + 1))
            CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep -v 'super\(.*|this\(.*|^$')
            ...
            do
                ...
                if [ $DETECTSUPER == "false" ]; then
                    ...
                    CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/""/g' | sed 's/^[[:space:]]*\/\/.*/;/g' | sed 's/\/\/.*/;/g' | egrep -v -n 'super\(.*|this\(.*')
                fi
                if [ $DEBUG == "true" ]; then
            ...
            if [ $DETECTSUPER == "true" -a -z "$(echo $CONTENTVAR | grep '}')" ]; then
                CONTENTVAR=$(sed -n "${LINE}p" $FILE | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep ';[[:space:]]*$')
                while [ -z "$CONTENTVAR" ]
                do
                    if [ ! -z "$(sed -n "${LINE}p" $FILE | sed 's/"[^"]*"//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep '^[^;]*{')" ]; then
            ...
            LINE=$((${LINE} - 1))
            CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep -v '^[[:space:]]*return([[:space:]]+|[[:space:]]*[;\({])|throw[[:space:]]+.*')
            if [ ! -z "$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep '^[[:space:]]*return([[:space:]]+|[[:space:]]*[;\({])|throw[[:space:]]+.*')" ]; then
                DETECTRETURN="true"
                LINE=$((${LINE} - 1))
            else
                DETECTRETURN="false"
            ...
            do
                ...
                if [ $DETECTRETURN == "false" ]; then
                    ...
                    CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*/;/g' | sed 's/\/\/.*//g' | egrep -v -n '^[[:space:]]*return([[:space:]]+|[[:space:]]*[;\({])|throw[[:space:]]+.*')
                fi
                if [ $DEBUG == "true" ]; then
            ...
            if [ "$DETECTRETURN" == "true" -o ! -z "$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep ';[[:space:]]*$')" ]; then
                UNREACHEDABLE="false"
            else
                UNREACHEDABLE="unknow"
            fi

            if [ -z "$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep '}' | grep -v ';[[:space:]]*$')" -a $DETECTRETURN == "false" ]; then
                if [ ! -z "$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep '[})].*;[[:space:]]*$')" ]; then
                    bracelocation=$LINE
                    braceencounter="true"
                    if [ ! -z "$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep ')[^}]*;[[:space:]]*$')" ]; then
                        # echo "find leftparentheses"
                        # sed -n "${LINE}p" $FILE
                ...
                LINE=$((${LINE} + 1))
                COUNT=$(($COUNT - 1))
                CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep '^[[:space:]]*return([[:space:]]+|[[:space:]]*[;\({])|throw[[:space:]]+.*')
        ...
        # STARTLINE=$LINE
        LINE=$(($LINE - 1))
        CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep -v '^[[:space:]]*$' | egrep '[;{}][[:space:]]*$')
        while [ -z "$CONTENTVAR" ]
        do
            LINE=$(($LINE - 1))
            CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep -v '^[[:space:]]*$' | egrep -n '[;{}][[:space:]]*$')
        done
        LINE=$(($LINE + 1))
        CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep -v '^[[:space:]]*$')
        while [ -z "$CONTENTVAR" ]
        do
            LINE=$(($LINE + 1))
            CONTENTVAR=$(sed -n "${LINE}p" ${FILE} | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g')
        done
        STARTLINE=$LINE
    fi
}
...
function PrintMessage() {
    ...
    echo -e "${count}\tFilename = ${FILENAME}" >> ${LOGFILE}
    echo -e "\tClassname = ${CLASSNAME}" >> ${LOGFILE}
    echo -e "\tMethodname = ${CONSTRUCTORPREFIX}${METHODNAME}" >> ${LOGFILE}
    echo -e "\tGreppattern = \"${GREPPAT}\"" >> ${LOGFILE}
    echo -e "\tMatchedLines = ${WL}" >> ${LOGFILE}
    echo -e $msg >> ${LOGFILE}
...
function filehandle() {
    ...
    wl=$(sed -n '1,$p' $file | grep '}.*;[[:space:]]*$' | grep -v 'hejinyi' | wc -l | awk '{print $1}')
...
function checkLastCodeBlock() {
    ...
    if [ ! -z "$(sed -n "${STARTLINEFORCHECKREACHEDABLE}p" $file | grep '[[:space:]]*while[[:space:]]*([[:space:]]*true[[:space:]]*)')" ]; then
        ...
        return
    elif [ -z "$(sed -n "${STARTLINEFORCHECKREACHEDABLE},${ENDLINEFORCHECKREACHEDABLE}p" $file | sed 's/"[^"]*"/ /g' | sed 's/\/\/.*//g' | egrep '^[[:space:]]*return([[:space:]]+|[[:space:]]*[;\({])|throw[[:space:]]+.*')" ]; then
        if [ -z "$(sed -n "${STARTLINEFORCHECKREACHEDABLE}p" $file | grep 'finally')" ]; then
            ...
            if [ -z "$(sed -n "${STARTLINEFORCHECKREACHEDABLE},${ENDLINEFORCHECKREACHEDABLE}p" $file | sed 's/"[^"]*"/ /g' | sed 's/\/\/.*//g' | egrep '\.close\(.*\)|\.endTransaction\(.*\)')" ]; then
                UNREACHEDABLE="false"
                if [ $DEBUG == "true" ]; then
    ...
    if [ $countbraces -eq 1 ]; then
        if [ ! -z "$(sed -n "${STARTLINEFORCHECKREACHEDABLE}p" $file | grep 'else')" ]; then
            # if [ -z "$(sed -n "${STARTLINEFORCHECKREACHEDABLE}p" $file | egrep 'else[[:space:]]+if')" ]; then
                # detectelse="true"
            # fi
            ...
            # if [ "$detectelse" == "false" ]; then
                # echo "detectelse = $detectelse, old unreachedable = $UNREACHEDABLE, new unreachedable = $UNREACHEDABLE"
                # UNREACHEDABLE="false"
            # fi
            return
        ...
        do
            if [ ! -z "$(sed -n "${line}p" $file | grep ';[[:space:]]*$')" ]; then
                ...
                contentvar=$(sed -n "${innerline}p" $file | egrep -n '[;}{]|^$')
                while [ -z "$contentvar" ]
                do
                    innerline=$(($innerline - 1))
                    contentvar=$(sed -n "${innerline}p" $file | egrep -n '[;}{]|^$')
...
function addLogToMethod() {
    ...
    if [ ${ISABSTRACTMETHOD} == "true" ]; then
        ...
        sed -i "${STARTLINE}a ${PREFIX}Log.d(\"${TAG}\", \"*****##### Enter<garymethodline> ${CONSTRUCTORPREFIX}${CLASSNAME}.${METHODNAME} #####*****\");" $1
        sed -i "${ENDLINE}i ${PREFIX}Log.d(\"${TAG}\", \"*****##### Leave<garymethodline> ${CONSTRUCTORPREFIX}${CLASSNAME}.${METHODNAME} #####*****\");" $1

        # LINE=$(sed -n "${STARTLINE},${ENDLINE}p" $1 | egrep -n '^[[:space:]]*(return;|return[[:space:]]+.*;|return\(.*\).*;|return.*{.*}|throw[[:space:]]+.*)' | sed -n '1p' | cut -d ':' -f 1)
        RETURNCOUNT=$(sed -n "${STARTLINE},${ENDLINE}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep '^[[:space:]]*return([[:space:]]+|[[:space:]]*[;\({])|throw[[:space:]]+.*' | wc -l)
        if [ $DEBUG == "true" ]; then
            echo "addLogToMethod: find $RETURNCOUNT inner return from $STARTLINE to $ENDLINE"
        fi
        echo "addLogToMethod: find $RETURNCOUNT inner return from $STARTLINE to $ENDLINE" >> $LOGFILE

        declare -i localcount=0
        while [ $localcount -ne $RETURNCOUNT ]
        do
            if [ -z "$(sed -n "${STARTLINE},${ENDLINE}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep '^[[:space:]]*return([[:space:]]+|[[:space:]]*[;\({])|throw[[:space:]]+.*')" ]; then
            ...
            LINEBACKUP=$LINE
            LINE=$(($LINE - 1))
            CONTENTVAR=$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g')
            while [ -z "$CONTENTVAR" ]
            do
                LINE=$(($LINE - 1))
                CONTENTVAR=$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g')
            done
            if [ -z "$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep '[;}{][[:space:]]*$|case.*:|default:[[:space:]]*')" ]; then
                echo "addLogToMethod: need to add {/} for return of $METHODNAME at line $LINE($localcount/$RETURNCOUNT)" >> $LOGFILE
                LINE=$LINEBACKUP
                workoutIndent $1 $LINE
                echo "addLogToMethod: { for return of $METHODNAME at line $LINE($localcount/$RETURNCOUNT)" >> $LOGFILE
                sed -i "${LINE}i ${PREFIX}{\t// added by LogHelpler from gary -$(date +%F) $(date +%T)-" $1
                LINE=$(($LINE + 1))
                LINEBACKUP=$LINE
                ENDLINE=$(($ENDLINE + 1))
                CONTENTVAR=$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep '[{]')
                if [ ! -z "$(echo $CONTENTVAR | grep '{')" ]; then
                    findBrace rightbrace $1 $LINE
                    LINE=$GLOBALLINE
                fi
                CONTENTVAR=$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep ';[[:space:]]*$')
                while [ -z "$CONTENTVAR" ]
                do
                    if [ ! -z "$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep '^[^;]*{')" ]; then
                        findBrace rightbrace $1 $LINE
                        LINE=$GLOBALLINE
                        LINE=$(($LINE - 1))
                    fi
                    echo "addLogToMethod: return of $METHODNAME at line $LINE($localcount/$RETURNCOUNT) is multiline" >> $LOGFILE
                    LINE=$(($LINE + 1))
                    CONTENTVAR=$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep ';[[:space:]]*$')
                done
            ...
            echo "addLogToMethod: handle return of $METHODNAME at line $LINE($localcount/$RETURNCOUNT)" >> $LOGFILE
            sed -i "${LINE}s/return/garyturn/g" $1
            workoutIndent $1 $LINE
            sed -i "${LINE}i ${PREFIX}Log.d(\"${TAG}\", \"*****##### Leave<garymethodline> ${CLASSNAME}.${METHODNAME} #####*****\");" $1
            ENDLINE=$(($ENDLINE + 1))
        done
        ...
        if [ $RETURNCOUNT -gt 0 ]; then
            ...
            echo "Returncount = $RETURNCOUNT" >> $LOGFILE
            if [ $UNREACHEDABLE == "unknow" ]; then
                declare tmpcheckreachedablefile="/tmp/.checkreachedablefile"
            ...
            if [ $UNREACHEDABLE == "unknow" ]; then
                sed -i "${ENDLINE}s/##### Leave/##### maybeunreachedableLeave/g" $1
...
# Time: 2013-12-08
###############################################################
function workoutIndent1() {
    local LINE
    local LOCALCONTENTVAR
    local tmpIndentFile="/tmp/.hejinyiLogHelplertmpIndentFile"
    local -i COUNTS
    local -i countindent
    # local -i wl
    PREFIX=""
    LINE=$2
    LOCALCONTENTVAR=$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | sed '/^$/d')
    ...
    do
        ...
        echo "while 8, try to workout the Indent for line $2 (Tomethod: $METHODNAME, Line = $LINE)" >> $LOGFILE
    done
    sed -n "${LINE}p" $1 | sed 's/ /\n/g' > $tmpIndentFile
    sed -i '$a stop' $tmpIndentFile
    COUNTS=1 
    LOCALCONTENTVAR=$(sed -n "${COUNTS}p" $tmpIndentFile)
    # wl=$(wc -l $tmpIndentFile | awk '{print $1}')
    # while [ -z "$LOCALCONTENTVAR" -a $COUNTS -le $wl ]
    while [ -z "$LOCALCONTENTVAR" ]
    do
        COUNTS=$(($COUNTS + 1))
        LOCALCONTENTVAR=$(sed -n "${COUNTS}p" $tmpIndentFile)
        PREFIX=$(echo "${PREFIX}MYPREFIX")
        countindent=$(($COUNTS - 1))
        if [ $DEBUG == "true" ]; then
            echo "while 9, count the blank (Count is now $countindent)"
        fi
        echo "while 9, count the blank (Count is now $countindent)" >> $LOGFILE
    done
    LOCALCONTENTVAR=$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep -v 'new .*(.* {' | grep '{')
    if [ ! -z "${LOCALCONTENTVAR}" ]; then
        ...
        echo "additional 4 whitespaces is added, because we count the whitespace of a definition of a method" >> $LOGFILE
    fi
    # rm -f $tmpIndentFile
}
...
function workoutIndent() {
    ...
    # local oldBeginWord=$(sed -n "${LINE}p" $1 | sed 's/$/end/g' | awk '{print $1}')
    # local newBeginWord=$(echo "hejinyi$oldBeginWord")
    COUNTS=$(($(sed -n "${LINE}p" $1 | sed 's/[^[:space:]]/a/g' | sed 's/$/end/g' | sed 's/a.*end//g' | wc -m) - 1))
    if [ ! -z "$(sed -n "${LINE}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep -v 'new .*(.* {' | grep '{')" ]; then
        COUNTS=$(($COUNTS + 4))
        if [ $DEBUG == "true" ]; then
            echo "additional 4 whitespaces is added, because we count the whitespace of a definition of a method"
        fi
...
do
    ...
    GREPPAT=$(echo ${ONELINE} | sed 's/.*\/\^//g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\$\/.*//g' | sed 's/\[/\\\[/g' | sed 's/\]/\\\]/g')
    GREPPAT=${GREPPAT#* }
    GREPPAT=${GREPPAT%* }
    WL=$(grep "${GREPPAT}" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "${GREPPAT}" | wc -l) 
    ...
    if [ ${WL} -gt 1 ]; then
        ...
        if [ $ISINTERFACE == "true" ]; then
            ISINTERFACE="false"
            LOCALWL=$(grep "interface $PRIVATECLASS" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "interface $PRIVATECLASS" | wc -l)
            if [ $LOCALWL -ne 1 ]; then
                RESULT="!!! Failed to locate this method (${PRIVATECLASS}.${METHODNAME}) !!!\n"
                count=$((${count} + 1))
                continue
            else
                RESULT="*** Located this methos successed! ***\n"
            fi
            SINGLELINE=$(grep -n "interface $PRIVATECLASS" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "interface $PRIVATECLASS" | cut -d ':' -f 1)
        elif [ $ISCLASS == "true" ]; then
            ISCLASS="false"
            LOCALWL=$(grep "class $PRIVATECLASS" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "class $PRIVATECLASS" | wc -l)
            if [ $LOCALWL -ne 1 ]; then
                RESULT="!!! Failed to locate this method!!! (${PRIVATECLASS}.${METHODNAME}) ***\n"
                count=$((${count} + 1))
                continue
            ...
            SINGLELINE=$(grep -n "class $PRIVATECLASS" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "class $PRIVATECLASS" | cut -d ':' -f 1)
        fi
        PrintMessage msg1
        findBlock classBlock $1 $SINGLELINE
        if [ $STARTLINE -eq $ENDLINE ]; then
            echo "!!! a singleline class !!!"
            echo "!!! a singleline class !!!" >> $LOGFILE
            count=$((${count} + 1))
            continue
        fi
        LOCALWL=$(sed -n "${STARTLINE},${ENDLINE}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "$GREPPAT" | wc -l)
        if [ $LOCALWL -le 1 ]; then
            SINGLELINE=$(sed -n "${STARTLINE},${ENDLINE}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep -n "$GREPPAT" | cut -d ':' -f 1)
        else
            if [ $DEBUG == "true" ]; then
                echo "!!! There also more than one prototype for this method in inner class ($PRIVATECLASS) !!!"
            fi
            echo "!!! There also more than one prototype for this method in inner class ($PRIVATECLASS) !!!" >> $LOGFILE
            CONTENTVAR=$(sed -n "${STARTLINE},${ENDLINE}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep -n "$GREPPAT" | sed 's/:.*//g')
            declare -i localSTARTLINE=$STARTLINE
            SINGLELINE=0
            for var in $CONTENTVAR
            do
                STARTLINE=$localSTARTLINE
                if [ -z "$(sed -n "${STARTLINE},${var}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "new .*(.*{")" ]; then
                    SINGLELINE=$var
                    break
                else
                    declare innervar
                    ...
                    do
                        innervar=$(sed -n "${innersingleline}p" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "new .*(.*{") 
        ...
        echo "!!! Can't locate this method, there are no definition for this method in ($PRIVATECLASS) !!!" >> $LOGFILE
        ...
        SINGLELINE=$(grep -n "$GREPPAT" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep "$GREPPAT" | cut -d ':' -f 1)
        findBlock methodBlock $1 $SINGLELINE
        addLogToMethod $1
    fi
    count=$((${count} + 1))
...
declare tmpCLASSNAMECOLLECTION=$(egrep 'class .*{' $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | sed 's/.*class//g' | awk '{print $1}' | sed 's/\n/ /g')
...
if [ ! -z "$CLASSNAMECOLLECTION" ]; then
    ...
    do
        WL=$(egrep "class $var" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep "class $var" | wc -l)
        if [ $WL -lt 1 ]; then
            if [ $DEBUG == "true" ]; then
                tput setf 4
                echo "!!! Can't find class $var, maybe it is an interface !!!"
                echo "!!! Can't find class $var, maybe it is an interface  !!!" >> $LOGFILE
            ...
            while [ $count -le $WL ]
            do
                SINGLELINE=$(egrep -n "class $var" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep "class $var" | sed -n "${count}p" | cut -d ':' -f 1)
                findBlock classBlock $1 $SINGLELINE "startline"
                workoutIndent $1 $SINGLELINE
                sed -i "${STARTLINE}a ${PREFIX}{Log.d(\"$TAG\", \"*****##### Create<garycreateline> an instance of type $var #####*****\");}" $1
                count=$((${count} + 1))
            done
        fi
        if [ $DEBUG == "false" -a $RECURSION -eq 0 ]; then
...
echo "***** begin to handle anonymousclass *****" >> $LOGFILE
declare ANONYMOUSINNERCLASS
ANONYMOUSINNERCLASS=$(egrep 'new[[:space:]]+.*\(.*\)[[:space:]]*{[[:space:]]*$' $1 | grep -v 'new[[:space:]]*.*;' | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep 'new[[:space:]]+.*\(.*\)[[:space:]]*{[[:space:]]*$' | awk '{print $(NF-1)}' | sed 's/(.*)//g')
declare -i countsum=$(echo $ANONYMOUSINNERCLASS | awk '{print NF}')
echo "***** find out $countsum anonymousclasses, they are: $ANONYMOUSINNERCLASS *****" >> $LOGFILE
declare -i innercount=0
if [ $DEBUG == "false" -a $RECURSION -eq 0 ]; then
    tput setf 3
...
if [ ! -z "$ANONYMOUSINNERCLASS" ]; then
    ...
    do
        ...
        PRIVATECLASS=${LOOPER}
        LOOPER=$(echo $LOOPER | sed 's/(/\\(/g' | sed 's/)/\\)/g' | sed 's/\[/\\\[/g' | sed 's/\]/\\\]/g')
        CONTENTVAR=$(egrep "new[[:space:]]+${LOOPER}\(.*\)[[:space:]]*{" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep "new[[:space:]]+${LOOPER}\(.*\)[[:space:]]*{")
        if [ -z "$CONTENTVAR" ]; then
            if [ $DEBUG == "true" ]; then
                tput setf 4
            ...
            echo "!!! Can't find new ${LOOPER}() { !!!" >> $LOGFILE
            continue
        fi
        SINGLELINE=$(egrep -n "new[[:space:]]+${LOOPER}\(.*\)[[:space:]]*{" $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | egrep "new[[:space:]]+${LOOPER}\(.*\)[[:space:]]*{" | sed -n '1p' | cut -d ':' -f 1)
        if [ $DEBUG == "true" ]; then
        ...
        if [ $tmpvar -le $STARTLINE ]; then
            tput setf 4
            echo "!!! Anonymousinnerclass $LOOPER at line $SINGLELINE is a blank class !!!"
            echo "!!! Anonymousinnerclass $LOOPER at line $SINGLELINE is a blank class !!!" >> $LOGFILE
            tput setf 7
            continue
        fi
        unset tmpvar
        ...
        NEWFILE="/tmp/${LOOPER}_${STARTLINE}_${ENDLINE}.java"
        ...
        echo "anonymousclass new ${LOOPER} () {, Startline = $STARTLINE, Endline = $ENDLINE" >> $LOGFILE
        ...
        STARTLINE=$(($STARTLINE - 1))
        ENDLINE=$(($ENDLINE + 1))
        # LINESOFNEWFILE=$(wc -l $1 | awk '{print $1}')
...
if [ $RECURSION -eq 0 ]; then
    ...
    if [ ! -z "$(grep 'Log\.d(".*", ".*##### .* #####.*");' $1)" ]; then
        if [ -z "$(grep 'import android\.util\.Log;' $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep 'import android\.util\.Log;')" ]; then
            declare -i line
            declare UPORDOWN
            if [ ! -z "$(grep -n 'package.*;' $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep 'package.*;')" ]; then
                line=$(grep -n 'package.*;' $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep 'package.*;' | sed -n '1p' | cut -d ':' -f 1)
                UPORDOWN="down"
            elif [ ! -z "$(grep -n 'import.*;' $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep 'import.*;')" ]; then
                line=$(grep -n 'import.*;' $1 | sed 's/"[^"]*"/ /g' | sed 's/^[[:space:]]*\/\/.*//g' | sed 's/\/\/.*//g' | grep 'import.*;' | sed -n '1p' | cut -d ':' -f 1)
                UPORDOWN="down"
            else
                line=1
                UPORDOWN="up"
    ...
    while [ ! -z "$(grep '##### maybeunreachedableLeave' $1)" ]
    do
        if [ $DEBUG == "true" ]; then
            echo "while 12, find out unreachedable statement in $1"
        fi
        LINE=$(grep -n '##### maybeunreachedableLeave' $1 | sed -n '1p' | cut -d ':' -f 1)
        findBlock inclassBlock $1 $LINE
        sed -i "${LINE}s/##### maybeunreachedableLeave/##### Leave/g" $1
    ...
    if [ -z "$ISDIRECTROY" ]; then
        ...
        rm -f /tmp/.hejinyiLogHelplertmp
        touch /tmp/.hejinyiLogHelplertmp
        awk '{gsub(/garymethodline|garycreateline/,NR)}{print $0}' $1 >> /tmp/.hejinyiLogHelplertmp
        mv /tmp/.hejinyiLogHelplertmp $1
    fi
    ENDTIME=$(date +%s)
    tput setf 2
    echo "TotalTime: $((($ENDTIME - $STARTTIME) / 3600))h $((($ENDTIME - $STARTTIME) % 3600 / 60))m $((($ENDTIME - $STARTTIME) % 60))s"
    echo "TotalTime: $((($ENDTIME - $STARTTIME) / 3600))h $((($ENDTIME - $STARTTIME) % 3600 / 60))m $((($ENDTIME - $STARTTIME) % 60))s" >> $LOGFILE
    tput setf 7
    unset LINEREGISTER
    unset LINETOCUT
fi
